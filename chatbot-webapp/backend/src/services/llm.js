...
/**
 * Future implementation sketch (do not implement yet):
 *
 * class LLMChatService {
 *   async generateResponse({ requestId, user, message, conversation }) {
 *     // call an LLM API here and return the reply text
 *   }
 * }
 *
 * Swapping ConsoleChatService for LLMChatService in services/chat.js is
 * the only change required — routes, schema, and auth stay untouched.
 */
class LLMChatService {
  async generateResponse({ requestId, user, message, conversation }) {
